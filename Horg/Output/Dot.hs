...
showHeadings conf hs =
    T.concat $ [T.pack "digraph G {",
...
showEdge d
    ...
    | kind d == Header2Subheader = showEdgeWith (T.pack "[color=red];") d
    | kind d == Tag2Header = showEdgeWith (T.pack "[color=green];") d
    | kind d == Header2Content = showEdgeWith (T.pack "[color=black];") d
    | otherwise = T.empty
